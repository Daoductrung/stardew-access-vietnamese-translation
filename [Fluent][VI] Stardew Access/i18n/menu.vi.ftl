...
common-ui-next_page_button = Nút Trang tiếp theo
...
options_element-dropdown_info = Hộp chọn {$label}, đã chọn tùy chọn {$selected_option}
options_element-slider_info = Thanh trượt {$slider_value}% {$label}
...
menu-tile_info-select_marking_index = Chọn chỉ mục
...
menu-tile_info-edit_existing_data = Sửa dữ liệu?
...
menu-tile_data_entry-tile_name_empty = Tên ô không được để trống hoặc rỗng
menu-tile_data_entry-categories_drop_down_label = Danh mục
menu-tile_data_entry-mod_dependency_drop_down_label = Phụ thuộc vào mod
menu-tile_data_entry-event_check_box_label = Kiểm tra {$is_festival ->
    [0] {$is_wedding ->
        [0] sự kiện,
       *[1] đám cưới,
      }
   *[1] lễ hội hiện tại: {$festival_name},
  } id: {$event_id}
menu-tile_data_entry-farm_type_check_box_label = Kiểm tra loại nông trại hiện tại: {$farm_type}
menu-tile_data_entry-farm_house_upgrade_level_drop_down_label = Kiểm tra cấp nâng cấp nhà
...
menu-donation_common-donatable_item_in_inventory-prefix = Có thể quyên góp {$content}
...
menu-collections_page-unshipped = Chưa gửi hàng
...
menu-crafting_page-next_recipe_list_button = Nút danh sách công thức tiếp theo

### Exit Page

menu-exit_page-exit_to_title_button = Nút Thoát về màn hình chính
...
menu-skills_page-player_info = {$name}, {$title}{$golden_walnut_count ->
    [0] {EMPTYSTRING()}
    [1] , 1 Óc chó vàng
   *[other] , {$golden_walnut_count} Óc chó vàng
  }{$qi_gem_count ->
    [0] {EMPTYSTRING()}
    [1] , 1 Đá quý Qi
   *[other] , {$qi_gem_count} Đá quý Qi
  }{$house_upgrade_level ->
    [1] {EMPTYSTRING()}
   ...
   *[other] , nhà đã nâng cấp lên cấp {$house_upgrade_level}
    ...
    [0] {EMPTYSTRING()}
   *[other] , tầng mỏ thấp nhất đã đến: {$lowest_mine_level}
  }{$stardrop_count ->
    [0] {EMPTYSTRING()}
    [1] , 1 Quả Sao
   *[other] , {$stardrop_count} Quả Sao
  }{$mastery_level ->
    [-1] {EMPTYSTRING()}
   *[other] , Cấp Tinh thông {$mastery_level}, {$current_mastery_points} trên {$required_mastery_points} điểm
  }
menu-skills_page-skill_info = {$name} ở cấp {$level},
  {$buffs}
...
menu-animal_page-animal_info = {$name}, {$type}{$heart_count ->
    [-1] {EMPTYSTRING()}
    [1] , 1 tim
   *[other] , {$heart_count} tim
  }{$has_been_pet ->
    [0] , chưa được vuốt ve
   *[other] {EMPTYSTRING()}
  }{$has_received_animal_cracker ->
    [0] {EMPTYSTRING()}
   *[other], đã ăn bánh quy động vật
  }
...
menu-forge-start_forging_button = Nút Bắt đầu rèn{$forge_cost ->
    [0] {EMPTYSTRING()}
   *[other] , tốn {$forge_cost} Mảnh Vụn Than
  }
menu-forge-unforge_button = Nút Hủy rèn
menu-forge-weapon_input_slot = {$is_empty ->
    [0] Ô vũ khí: {$item_name}
   *[1] Đặt vũ khí, công cụ hoặc nhẫn ở đây
  }
menu-forge-gemstone_input_slot = {$is_empty ->
    [0] Ô đá quý: {$item_name}
   *[1] Đặt đá quý hoặc nhẫn ở đây
  }

### Geode Menu

menu-geode-geode_input_slot = Đặt địa chất ở đây
menu-geode-received_treasure_info = Nhận được {$treasure_name}

### Item Grab Menu

menu-item_grab-last_shipped_info = Gửi hàng lần cuối: {$shipped_item_name}
...
menu-tailoring-cloth_input_slot = {$is_empty ->
    [0] Ô vải: {$item_name}
   *[1] Đặt vải hoặc quần áo có thể nhuộm ở đây
  }
menu-tailoring-spool_slot = {$is_empty ->
    [0] Cuộn chỉ: {$item_name}
   *[1] Đặt nguyên liệu ở đây
  }
...
menu-animal_query-confirm_selling_button = Nút xác nhận bán động vật
menu-animal_query-cancel_selling_button = Nút hủy bán động vật
menu-animal_query-selling_button = Nút Bán với giá {$price}g
menu-animal_query-move_home_button = Nút đổi nhà
menu-animal_query-text_box = Hộp văn bản tên động vật
...
menu-building_skin-skin_info = {$type ->
    [cabin] {$index ->
        ...
        [5] {$id}
        [6] {$id}
       *[other] Skin Nhà không xác định, id:{$id}, index:{$index}
      }
    [pet_bowl] {$index ->
        [0] Bát thú cưng mặc định
        [1] {$id}
        [2] {$id}
       *[other] Skin Bát thú cưng không xác định, id:{$id}, index:{$index}
      }
   *[other] {$index}: {$id}
  }
menu-building_skin-next_skin_button = Skin tiếp theo
menu-building_skin-previous_skin_button = Skin trước
...
menu-confirmation_dialogue-copy_button = {$dialogue_message}
  Nút Sao chép vào bộ nhớ tạm

### Item List Menu

menu-item_list-ok_button = {$title}
  {$item_list}
  Trang {$current_page} của {$total_pages}
  Nút Ok
...
menu-letter_viewer-letter_message = {$message_content}{$is_money_included ->
   ...
   *[1] , {$learned_recipe}
    ...
    [0] {EMPTYSTRING()}
   *[1] , Nhấp chuột trái để nhận nhiệm vụ
  }
menu-letter_viewer-pagination_text-prefix = Trang {$current_page} của {$total_pages}
  {$content}
menu-letter_viewer-grabbable_item_text = Nhấp chuột trái để nhận {$name}
menu-letter_viewer-image_note = {$note_id ->
    [11] Hình ảnh Marnie trẻ tuổi nắm tay Jas lúc còn bé. Họ được bao quanh bởi các động vật trong trang trại.
    [16] Một bản đồ kho báu mô tả một tảng đá lớn ở phía Tây Bắc của đường ray. Một dấu X đỏ được đặt ở bên phải tảng đá.
    [17] Một bản đồ kho báu mô tả con sông ở phía bắc Siêu thị Joja với một dấu X đỏ ở góc Đông Bắc nhất.
    [18] Một bản đồ kho báu mô tả một chiếc ghế dài ở góc Đông Nam của sa mạc Calico, có một dấu X đỏ ở phía tây nam.
    [19] Một mô tả về nhà số 1 đường Willow với một chuỗi mũi tên. Một hình vuông đánh dấu khoảng trống trước cửa trước. Trình tự là trái, lên, phải, lên, phải, xuống, trái, xuống, trái, xuống, dấu tích.
    [20] một mô tả về quảng trường thị trấn với một chuỗi mũi tên. Mô tả ghi chú một khu vực ở chính giữa quảng trường. Trình tự như sau, phải, xuống, phải, lên, phải, lên, phải, xuống, trái, lên, trái, lên, phải, lên, trái, lên, trái, dấu tích.
    [21] Một mô tả về bụi cây lớn ở phía tây bắc của cây cầu ra bãi biển, vào ban đêm. Một chiếc đồng hồ hiển thị thời gian 12:40.
...
menu-pond_query-change_netting_button = Nút đổi lưới
menu-pond_query-empty_pond_button = Nút dọn ao
...
menu-purchase_animal-first_time_in_menu_info = Nhập tên của động vật vào hộp văn bản tên.
menu-purchase_animal-random_name_button = Nút tên ngẫu nhiên
menu-purchase_animal-animal_name_text_box = Hộp văn bản tên{$value ->
    [null] {EMPTYSTRING()}
   *[other] , Giá trị: {$value}
  }
...
menu-shipping-total_money_received_info = Đã nhận tổng cộng {$money}g. Nhấp chuột trái để lưu.
menu-shipping-money_received_from_category_info = Đã nhận {$money}g từ {$category_name}.
...
menu-billboard-daily_quest-accept_quest-suffix =
  Nhấp chuột trái để nhận nhiệm vụ.
...
menu-quest_log-quest_brief = {$name} {$is_completed ->
    [0] {SIGNOFNUMBER($days_left) ->
        [positive] , còn {$days_left} {$days_left ->
            [1] ngày
           *[other] ngày
          }
       *[other] {EMPTYSTRING()}
...
menu-special_orders_board-accept_button = Nhiệm vụ {$is_left_quest ->
  ...
  Nhấp chuột trái để nhận nhiệm vụ này.
menu-special_orders_board-quest_in_progress = Đang tiến hành: {$quest_details}
...
menu-title-stardew_access_loaded = Stardew Access phiên bản {$version} đã được tải {$cheats ->
    [1] với mã gian lận được bật
   *[0] {EMPTYSTRING()}
  }
menu-title-click_to_skip = Nhấp chuột trái để bỏ qua màn hình chính
menu-title-new_game_button = Nút Chơi mới
menu-title-load_button = Nút Tải
menu-title-co_op_button = Nút Chơi mạng
...
menu-load_game-delete_farm_confirmation_text = Thực sự muốn xóa nông trại?
menu-load_game-farm_details = {$index ->
   ...
   *[other] {$index}
    ...
    [-1] {EMPTYSTRING()}
   *[other] {$money}g
  }, {$date}, đã chơi {$hours_played} giờ

### Co-op Menu

menu-co_op-join_lan_game_button = Tham gia qua mạng LAN
menu-co_op-host_new_farm_button = Tạo nông trại mới
menu-co_op-refresh_button = Nút Làm mới
menu-co_op-join_tab_button = Thẻ Tham gia {$is_selected ->
    [0] {EMPTYSTRING()}
   *[1] đã chọn
  }
menu-co_op-host_tab_button = Thẻ Tạo mới {$is_selected ->
    [0] {EMPTYSTRING()}
   *[1] đã chọn
  }
menu-co_op-friend_hosted_farm_details = {$farm_name}, Chủ sở hữu: {$owner_name}, {$date}
